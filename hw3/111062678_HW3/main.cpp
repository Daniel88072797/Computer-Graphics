...
struct model
{
	...
	vector<Shape> shapes;
	Vector3 light_move = Vector3(0, 0, 0);
...
GLfloat offset_x[7] = {0, 0, 0, 0, 0.5, 0.5, 0.5};
GLfloat offset_y[7] = { 0, -0.25, -0.5, -0.75, 0, -0.25, -0.5 };
...
vector<string> model_list{ "../TextureModels/Fushigidane.obj", "../TextureModels/Mew.obj","../TextureModels/ZEBRA.obj","../TextureModels/Zenigame.obj", "../TextureModels/laurana500.obj", "../TextureModels/Nala.obj", "../TextureModels/Square.obj" };
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	glUniform3f(iLocLightMove, models[cur_idx].light_move.x, models[cur_idx].light_move.y, models[cur_idx].light_move.z);

	glUniform1i(iLocCutoff, cutoff);
	glUniform1f(iLocIntensity, intensity);
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		if (mini_mode == 0) {
			...
			//cout << "minification: nearest" << endl;
		}
		else {
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR); 
			//cout << "minification: linear mipmap linear" << endl;
		}

		if (models[cur_idx].shapes[i].material.isEye == 1) {
			glUniform1i(iLocIsEye, 1);
		}
		else {
			glUniform1i(iLocIsEye, 0);
		}
		
		glUniform2f(iLocOffset, offset_x[models[cur_idx].cur_eye_offset_idx], offset_y[models[cur_idx].cur_eye_offset_idx]);
		...
		glUniform1i(iLocLightingModel, lighting_model); 
		...
		case GLFW_KEY_LEFT:
			if (models[cur_idx].hasEye == true) {
				if (models[cur_idx].cur_eye_offset_idx == 0) {
					models[cur_idx].cur_eye_offset_idx = models[cur_idx].max_eye_offset - 1;
				}
				else {
					models[cur_idx].cur_eye_offset_idx = (models[cur_idx].cur_eye_offset_idx - 1) % models[cur_idx].max_eye_offset;
				}
	...
	if (mouse_pressed) {
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case GeoRotation:
				...
				break;
			case LightEdit:
				models[cur_idx].light_move.x += -diff_x * (1.0 / 400.0);
				models[cur_idx].light_move.y += diff_y * (1.0 / 400.0);
			}
...
GLuint LoadTextureImage(string image_path)
{
	...
	stbi_uc *data = stbi_load(image_path.c_str(), &width, &height, &channel, require_channel); //return every pixel in the image
	if (data != NULL)
	{
		GLuint tex = 0;
		...
		glGenTextures(1, &tex); //generate texture names
		glBindTexture(GL_TEXTURE_2D, tex);  //bind a named texture to a texturing target

		//specify a two-dimensional texture image、根據指定的參數，生成一個2D紋理（Texture）
...
void LoadTexturedModels(string model_path)
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		if (materials[i].diffuse_texname.find("Eye") != std::string::npos) {
			material.isEye = 1; //contain
			tmp_model.hasEye = true;
		}
		else {
			material.isEye = 0; //not contatin
		}
...
void setUniformVariables()
{
	...
	iLocLightMove = glGetUniformLocation(program, "light_move");
	iLocLightingModel = glGetUniformLocation(program, "lighting_model");
	iLocCutoff = glGetUniformLocation(program, "cutoff");
	iLocIntensity = glGetUniformLocation(program, "intensity");

	// [TODO] Get uniform location of texture
	iLocIsEye = glGetUniformLocation(program, "is_eye");
	iLocT = glGetUniformLocation(program, "tex");
	iLocOffset = glGetUniformLocation(program, "offset");
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "111062678 HW3", NULL, NULL);
    if (window == NULL)
    {
