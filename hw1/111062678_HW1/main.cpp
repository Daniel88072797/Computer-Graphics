...
void setPerspective()
{
	//cout << "setPerspective" << endl;
	cur_proj_mode = Perspective;
	 //project_matrix [...] = ...
	//float A = (proj.right + proj.left) / (proj.right - proj.left);
	//float B = (proj.top + proj.bottom) / (proj.top - proj.bottom);
	//float C = (-1) * ( (proj.farClip + proj.nearClip) / (proj.farClip - proj.nearClip) );
	//float D = (-1) * ( (2 * proj.farClip * proj.nearClip) / (proj.farClip - proj.nearClip) );
	//  
	//project_matrix = Matrix4(
	//	(2 * proj.nearClip) / ((proj.right - proj.left) ), 0, A, 0,
	//	0, (2 * proj.nearClip) / ((proj.top - proj.bottom) ), B, 0,
	//	0, 0, C, D,
	//	0, 0, -1, 0
	//);
...
void drawPlane()
{
	...
	//Setup vertex buffer(from system memory to Graphics memory)
	glGenVertexArrays(1, &quad.vao);
	glGenBuffers(1, &quad.vbo);
	glGenBuffers(1, &quad.ebo); //setup EBO
...
void RenderScene(void) {	
	...
	mvp[1] = MVP[1];  mvp[5] = MVP[5];   mvp[9] = MVP[9];    mvp[13] = MVP[13];
	mvp[2] = MVP[2];  mvp[6] = MVP[6];   mvp[10] = MVP[10];   mvp[14] = MVP[14];
	mvp[3] = MVP[3];  mvp[7] = MVP[7];   mvp[11] = MVP[11];   mvp[15] = MVP[15];
	//mvp[0] = 1;  mvp[4] = 0;   mvp[8] = 0;    mvp[12] = 0.5;
...
void print_info() {
	...
	cout << rotateX(models[cur_idx].rotation[1]) * rotateY(models[cur_idx].rotation[0]) * rotateZ(models[cur_idx].rotation[2]) << endl;
...
//Apply change on Z axis when scrollthewheel
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	// [TODO] scroll up positive, otherwise it would be negtive
	//Vector3 move_z = Vector3(0, yoffset, 0 );
	...
	switch (cur_trans_mode) {
		...
		case GeoRotation:
			...
			break;
		case ViewCenter:
			main_camera.center = main_camera.center + move_z;
			setViewingMatrix();
			break;
		case ViewEye:
			main_camera.position = main_camera.position + move_z;
			setViewingMatrix();
			break;
		case ViewUp:
			main_camera.up_vector = main_camera.up_vector + move_z;
			setViewingMatrix();
			break;
