...
void setPerspective()
{
	...
	float f = 1 / (tanf((M_PI / 180.0) * proj.fovy / 2));
...
void RenderScene(void) {	
	...
	R = rotateX(models[cur_idx].rotation[1]) * rotateY(models[cur_idx].rotation[0]) * rotateZ(models[cur_idx].rotation[2]);

	Matrix4 model_transform_matrix = T * R * S;

	MVP = project_matrix * view_matrix * model_transform_matrix;

	// row-major ---> column-major
	setGLMatrix(mvp, MVP);
	setGLMatrix(m, model_transform_matrix);
	setGLMatrix(v, view_matrix);

	// use uniform to send mvp to vertex shader
	glUniformMatrix4fv(uniform.iLocMVP, 1, GL_FALSE, mvp);
	glUniformMatrix4fv(uniform.iLocModel, 1, GL_FALSE, m);
	glUniformMatrix4fv(uniform.iLocView, 1, GL_FALSE, v);

	//use uniform to send light index to vertex shader
	glUniform1i(uniform.iLocLightIdx, light_idx);
	glUniform1i(uniform.iLocShininess, shininess);
	glUniform3f(uniform.iLocLightMove, models[cur_idx].light_move.x, models[cur_idx].light_move.y, models[cur_idx].light_move.z);

	glUniform1i(uniform.iLocCutoff, cutoff);
	glUniform1f(uniform.iLocIntensity, intensity);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_Z:
			...
			break;
		case GLFW_KEY_X:
			if (cur_idx == 0) cur_idx = model_list.size() - 1;
			else cur_idx--;
			break;
		case GLFW_KEY_T:
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	Vector3 move_z = Vector3(0, 0, yoffset);
	move_z = move_z * 0.5;
	switch (cur_trans_mode) {

	case GeoTranslation:
		models[cur_idx].position += move_z;
		break;
	case GeoScaling:
		models[cur_idx].scale += move_z;
		break;
	case GeoRotation:
		models[cur_idx].rotation += move_z;
		break;
	case LightEdit:
		if (light_idx == 2) {
			cutoff += yoffset;
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	//Vector3 move_y = Vector3(0, ypos - p_ypos, 0);
...
void setShaders()
{
	...
	uniform.iLocModel = glGetUniformLocation(p, "m");
	uniform.iLocView = glGetUniformLocation(p, "v");
	uniform.iLocKa = glGetUniformLocation(p, "Ka");
	uniform.iLocKd = glGetUniformLocation(p, "Kd");
	uniform.iLocKs = glGetUniformLocation(p, "Ks");
	uniform.iLocLightIdx = glGetUniformLocation(p, "light_idx");
	uniform.iLocShininess = glGetUniformLocation(p, "Shininess");
	uniform.iLocLightMove = glGetUniformLocation(p, "light_move");
	uniform.iLocLightingModel = glGetUniformLocation(p, "lighting_model");
	uniform.iLocCutoff = glGetUniformLocation(p, "cutoff");
	uniform.iLocIntensity = glGetUniformLocation(p, "intensity");
